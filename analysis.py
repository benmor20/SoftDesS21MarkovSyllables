...
def analyze_markov_syllables(analyze_text, verbose=False, max_words=10000, min_syllables=1):
    ...
    Args:
        analyze_text: A string of text to split into syllables, generate words from
            the syllables, and return analysis for.
    ...
    """
    text = ""
    if analyze_text is list:
        for book in analyze_text:
            text += book + " "
    else:
        text = analyze_text
    analyze_text = text
    ...
    if verbose:
        print("Concatenated texts")

    sylls = markov.build_next_syllables(text)
    if verbose:
        print("Got syllables")
    all_words = set(markov.build_word_list(analyze_text))
    if verbose:
        print("Got word list")
    num_words = 0
    novel_words = set()
    ...
    syll_list = markov.build_syllable_list(analyze_text)
